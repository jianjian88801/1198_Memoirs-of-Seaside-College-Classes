<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="加入时间" prop="jiarushijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jiarushijian" label="加入时间" prop="jiarushijian">
              <el-input v-model="ruleForm.jiarushijian" 
                placeholder="加入时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.jiarushijian = this.getCurDateTime()
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiarubanji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiarubanjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
